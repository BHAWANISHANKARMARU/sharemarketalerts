...
try {
  ...
  const result = await send("Runtime.evaluate", {
    expression: `(async () => {
      for (let i = 0; i < 120 && !document.querySelector('[data-platform-products]'); i++) await new Promise(r => setTimeout(r, 50));
      await document.fonts.ready;
      const node = document.querySelector('[data-platform-products]');
