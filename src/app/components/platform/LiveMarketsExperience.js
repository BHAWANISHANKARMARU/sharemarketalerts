...
export default function LiveMarketsExperience() {
  ...
  const upCount = market.gainers.length;
  const downCount = market.losers.length;
  const upShare = Math.round((upCount / Math.max(upCount + downCount, 1)) * 100);

  const rows = useMemo(() => {
    let next = [...market.gainers, ...market.losers];
  ...
  }, [descending, market.gainers, market.losers, moverMode, signalFilter]);
    ...
    <main className={[s.workspacePage, s.liveWorkspace].join(" ")} data-live-terminal>
      ...
      <div className={s.wideCanvas}>
          ...
          <div className={s.screenerTableWrap}>
            <table className={s.screenerTable}>
              ...
              <tbody>{rows.map((quote, index) => {
                ...
                const relativeVolume = (1.08 + ((index * 17 + Math.abs(quote.changePercent || 0) * 13) % 145) / 100).toFixed(2);
                return <tr key={quote.symbol}><td><a href={quote.href} target="_blank" rel="noreferrer"><InstrumentMark symbol={quote.displaySymbol} tone={index} /><span><strong>{quote.displaySymbol}</strong><small>{quote.name}</small></span><em>NSE</em></a></td><td><strong>₹{quote.formattedValue}</strong></td><td><ChangeValue value={quote.formattedChange} direction={quote.direction} /></td><td><span className={s.volumeCell}><b style={{ width: `${Math.min(100, Number(relativeVolume) * 36)}%` }} />{relativeVolume}×</span></td><td><span className={signal.includes("buy") ? s.signalBuy : signal.includes("sell") ? s.signalSell : s.signalNeutral}>{signal}</span></td><td><span className={s.confidenceCell}><i><b style={{ width: `${confidence}%` }} /></i>{confidence}%</span></td><td><span className={s.marketCell}><i />India</span></td></tr>;
              })}</tbody>
            </table>
          </div>
          <footer><span>Showing {rows.length} of {market.gainers.length + market.losers.length} tracked equities</span><span>Relative volume and confidence are deterministic interface signals.</span></footer>
        </section>
        ...
        <section className={s.screenerSummary}>
          ...
          <article className={s.riskMonitor}><PanelHeading title="Risk monitor" subtitle="Session conditions" /><ul><li><span>Volatility</span><strong>Contained</strong><i data-level="low" /></li><li><span>Concentration</span><strong>Moderate</strong><i data-level="medium" /></li><li><span>Failed breaks</span><strong>Low</strong><i data-level="low" /></li><li><span>Liquidity</span><strong>Healthy</strong><i data-level="low" /></li></ul><div><span>SESSION MOMENTUM</span><strong>{market.momentumScore}<small>/100</small></strong><em>{market.momentumLabel}</em></div></article>
        </section>
        ...
        <p className={s.screenerDisclaimer}>Data source: {sources.yahoo.name} · {sources.yahoo.mode}. Live and fallback market values are informational; confirm price and suitability with your broker before acting.</p>
      </div>
    </main>
  );
}
